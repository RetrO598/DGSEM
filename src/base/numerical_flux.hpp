...
template <class T>
struct LaxFriedrichsFlux<equations::CompressibleNavierStokes3D<T>> {
  using traits =
      equations::EquationTraits<equations::CompressibleNavierStokes3D<T>>;
  using value_type = typename traits::value_type;
...
template <class T>
struct ChandrashekarFlux<equations::CompressibleNavierStokes3D<T>> {
  using traits =
      equations::EquationTraits<equations::CompressibleNavierStokes3D<T>>;
  constexpr static std::size_t NDIMS = traits::NDIMS;
  constexpr static std::size_t NVARS = traits::NVARS;
