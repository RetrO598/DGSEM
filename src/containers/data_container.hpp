...
struct StructuredContainerInitializer<T, Basis, Mapping, 1> {
  ...
  resize(std::size_t tot_elems, StructuredElementContainer<T, 1> &container) {
    ...
    Kokkos::realloc(container.left_neighbors, tot_elems, 1);
    Kokkos::realloc(container.jacobian_matrix, tot_elems, Basis::NNodes, 1, 1);
    Kokkos::realloc(container.contravariant_vectors, tot_elems, Basis::NNodes,
                    1, 1);
    Kokkos::realloc(container.inverse_jacobian, tot_elems, Basis::NNodes, 1);
  }

  KOKKOS_INLINE_FUNCTION constexpr static void
  calc_node_coordinates(std::size_t nelems, std::size_t ielem,
  ...
  KOKKOS_INLINE_FUNCTION constexpr static void
  calc_jacobian_matrix(std::size_t ielem, MatrixHost &jacobian,
                       const DataArrayHost &coordinates) {
    for (std::size_t i = 0; i < Basis::NNodes; ++i) {
      T tmp = 0.0;
  ...
  KOKKOS_INLINE_FUNCTION constexpr static void
  calc_inverse_jacobian(std::size_t ielem, DataArrayHost &inverse_jacobian,
  ...
  KOKKOS_INLINE_FUNCTION constexpr static void
  initialize_left_neighbor(std::size_t nelems, IndexArrayHost &left_neighbors,
