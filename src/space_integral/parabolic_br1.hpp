...
struct GradientVolumeIntegralFunctor<T, Basis, Equations, 3> {
  ...
  using VectorFieldArray = typename node_vector_field_type_traits<T, 3>::DataArray;
  using BasisData = typename Basis::DeviceData;

  GradientVolumeIntegralFunctor(DataArray u_, VectorFieldArray gradient_reference_,
                                const Equations& eq_)
      : u(u_), gradient_reference(gradient_reference_), eq(eq_),
        basis(Basis::device_data()) {}

  static void apply(DataArray u_, VectorFieldArray gradient_reference_,
                    const Equations& eq_,
                    const std::array<std::size_t, 3>& n_elems_) {
    GradientVolumeIntegralFunctor functor(u_, gradient_reference_, eq_);
    Kokkos::parallel_for(
        ...
        for (std::size_t inode = 0; inode < Basis::NNodes; ++inode) {
          ...
          for (std::size_t ii = 0; ii < Basis::NNodes; ++ii) {
            ...
            for (std::size_t var = 0; var < Equations::NGRAD_VARS; ++var) {
              gradient_reference(ielem, jelem, kelem, out_dof, var, 0) +=
          ...
          for (std::size_t jj = 0; jj < Basis::NNodes; ++jj) {
            ...
            for (std::size_t var = 0; var < Equations::NGRAD_VARS; ++var) {
              gradient_reference(ielem, jelem, kelem, out_dof, var, 1) +=
          ...
          for (std::size_t kk = 0; kk < Basis::NNodes; ++kk) {
            ...
            for (std::size_t var = 0; var < Equations::NGRAD_VARS; ++var) {
              gradient_reference(ielem, jelem, kelem, out_dof, var, 2) +=
...
struct GradientPhysicalTransformFunctor<T, Basis, Equations, 3> {
  ...
  GradientPhysicalTransformFunctor(VectorFieldArray gradient_reference_,
                                   VectorFieldArray gradient_,
                                   MetricArray contravariant_vectors_)
      : gradient_reference(gradient_reference_), gradient(gradient_),
        contravariant_vectors(contravariant_vectors_) {}

  static void apply(VectorFieldArray gradient_reference_,
                    VectorFieldArray gradient_,
                    MetricArray contravariant_vectors_,
                    const std::array<std::size_t, 3>& n_elems_) {
    GradientPhysicalTransformFunctor functor(gradient_reference_, gradient_,
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const std::size_t& ielem,
    ...
    for (std::size_t dof = 0; dof < ndofs; ++dof) {
      ...
      for (std::size_t var = 0; var < Equations::NGRAD_VARS; ++var) {
        const T ref_xi = gradient_reference(ielem, jelem, kelem, dof, var, 0);
        const T ref_eta = gradient_reference(ielem, jelem, kelem, dof, var, 1);
        const T ref_zeta =
            gradient_reference(ielem, jelem, kelem, dof, var, 2);
  ...
  KOKKOS_INLINE_FUNCTION void apply_face_x(std::size_t ielem, std::size_t jelem,
    ...
    for (std::size_t knode = 0; knode < Basis::NNodes; ++knode) {
      for (std::size_t jnode = 0; jnode < Basis::NNodes; ++jnode) {
        ...
        const auto q_left = load_gradient_variables(left_i, left_j, left_k, left_dof);
        const auto q_right = load_gradient_variables(ielem, jelem, kelem, right_dof);
  ...
  KOKKOS_INLINE_FUNCTION void apply_face_y(std::size_t ielem, std::size_t jelem,
    ...
    for (std::size_t knode = 0; knode < Basis::NNodes; ++knode) {
      for (std::size_t inode = 0; inode < Basis::NNodes; ++inode) {
        ...
        const auto q_top = load_gradient_variables(ielem, jelem, kelem, top_dof);

        for (std::size_t var = 0; var < Equations::NGRAD_VARS; ++var) {
          const T qhat = static_cast<T>(0.5) * (q_bottom[var] + q_top[var]);
          surface_flux(bottom_i, bottom_j, bottom_k,
                       face_dof(3, inode, knode), var) = qhat;
...
struct GradientSurfaceIntegralFunctor<T, Basis, Equations, 3> {
  ...
  GradientSurfaceIntegralFunctor(VectorFieldArray gradient_, DataArray surface_flux_,
                                 MetricArray contravariant_vectors_,
  ...
  static void apply(VectorFieldArray gradient_, DataArray surface_flux_,
                    ...
                    ScalarArray inverse_jacobian_,
                    const std::array<std::size_t, 3>& n_elems_) {
    GradientSurfaceIntegralFunctor functor(gradient_, surface_flux_,
                                           contravariant_vectors_,
                                           inverse_jacobian_);
    Kokkos::parallel_for(
    ...
    for (std::size_t knode = 0; knode < Basis::NNodes; ++knode) {
      for (std::size_t jnode = 0; jnode < Basis::NNodes; ++jnode) {
        ...
        add_face_contribution(ielem, jelem, kelem, left_dof,
                              face_dof(0, jnode, knode), left_normal, -factor_1);
        add_face_contribution(ielem, jelem, kelem, right_dof,
                              face_dof(1, jnode, knode), right_normal, factor_2);
      }
    }
    ...
    for (std::size_t knode = 0; knode < Basis::NNodes; ++knode) {
      for (std::size_t inode = 0; inode < Basis::NNodes; ++inode) {
        ...
        add_face_contribution(ielem, jelem, kelem, bottom_dof,
                              face_dof(2, inode, knode), bottom_normal, -factor_1);
    ...
    for (std::size_t jnode = 0; jnode < Basis::NNodes; ++jnode) {
      for (std::size_t inode = 0; inode < Basis::NNodes; ++inode) {
        ...
        add_face_contribution(ielem, jelem, kelem, back_dof,
                              face_dof(4, inode, jnode), back_normal, -factor_1);
        add_face_contribution(ielem, jelem, kelem, front_dof,
                              face_dof(5, inode, jnode), front_normal, factor_2);
      }
    }
  }
  ...
  face_normal(std::size_t ielem, std::size_t jelem, std::size_t kelem,
    ...
    const T sign = inverse_jacobian(ielem, jelem, kelem, dof) >= T{0} ? T{1}
                                                                       : T{-1};
    return {sign * contravariant_vectors(ielem, jelem, kelem, dof, direction, 0),
  ...
  add_face_contribution(std::size_t ielem, std::size_t jelem,
...
struct ViscousInterfaceFluxFunctor<T, Basis, Equations, 3> {
  ...
  ViscousInterfaceFluxFunctor(IndexArray neighbors_, VectorFieldArray viscous_flux_,
                              DataArray surface_flux_,
                              MetricArray contravariant_vectors_,
                              ScalarArray inverse_jacobian_)
  ...
  static void apply(IndexArray neighbors_, VectorFieldArray viscous_flux_,
                    ...
                    ScalarArray inverse_jacobian_,
                    const std::array<std::size_t, 3>& n_elems_) {
    ViscousInterfaceFluxFunctor functor(neighbors_, viscous_flux_, surface_flux_,
                                        contravariant_vectors_,
                                        inverse_jacobian_);
    Kokkos::parallel_for(
    ...
    for (std::size_t knode = 0; knode < Basis::NNodes; ++knode) {
      for (std::size_t jnode = 0; jnode < Basis::NNodes; ++jnode) {
        ...
        add_divergence_flux(left_i, left_j, left_k, face_dof(1, jnode, knode),
                            ...
                            face_dof(0, jnode, knode), right_dof,
                            normal_right);
      }
    }
  }
    ...
    for (std::size_t jnode = 0; jnode < Basis::NNodes; ++jnode) {
      for (std::size_t inode = 0; inode < Basis::NNodes; ++inode) {
        ...
        const auto normal_front =
            face_normal(ielem, jelem, kelem, front_dof, 2);
        add_divergence_flux(back_i, back_j, back_k,
                            face_dof(5, inode, jnode), back_dof, ielem, jelem,
                            kelem, face_dof(4, inode, jnode), front_dof,
                            normal_front);
      }
    }
  }
  ...
  dot_viscous_flux(std::size_t ielem, std::size_t jelem, std::size_t kelem,
                   std::size_t dof, std::size_t var,
                   const std::array<T, 3>& normal) const {
  ...
  add_divergence_flux(std::size_t secondary_i, std::size_t secondary_j,
    ...
    const std::array<T, 3> secondary_normal = {-primary_normal[0],
                                               -primary_normal[1],
                                               -primary_normal[2]};
    for (std::size_t var = 0; var < Equations::NVARS; ++var) {
      const T flux_primary =
          dot_viscous_flux(primary_i, primary_j, primary_k, primary_dof, var,
                           primary_normal);
      const T flux_secondary =
          dot_viscous_flux(secondary_i, secondary_j, secondary_k, secondary_dof,
                           var, secondary_normal);
      const T flux_hat = static_cast<T>(0.5) * (flux_primary + flux_secondary);
